...
momZinbinom2 <- function(sample) {
  error <- 0.001
  pi <- 0.5
  m <- mean(sample)
  v <- var(sample)
  zeroWeight <- sum(sample == 0 ) / length(sample)
  pi <- 0.5*zeroWeight
  prob <- 1/(v/m - m*pi/(1-pi))
  size <- prob*m/((1-pi)*(1-prob))
  prob <- max(error, min(prob, 1- error))
...
mleZinbinom2 <- function(sample) { 
  minLambda <- 0.01
  startParameters <- momZinbinom2(sample)
  fit <- maxLik(logLik = function(parameters) logLikelihoodZinbinom(sample, parameters),
                start = startParameters)
  return(fit)
}
...
zinbinomFitter2 <- function(sample){
  
  # Estimating the parameters
  
  mleFit <- mleZinbinom2(sample)
...
fakeSample <- rzinbinom(10, 4, 0.3, 0.7)

zinbinomFitter(fakeSample)
zinbinomFitter2(fakeSample)
